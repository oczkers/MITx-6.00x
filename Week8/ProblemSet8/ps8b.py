    ...
    def update(self):
        ...
        for virus in self.viruses:
            if not virus.doesClear():
                viruses_new.append(virus)

        pop_density = float(len(viruses_new)) / self.maxPop

        for virus in viruses_new:
            if pop_density >= 1:
                break
            try:
                viruses_new.append(virus.reproduce(pop_density))
            except NoChildException:
                pass

        self.viruses = viruses_new
        return len(self.viruses)
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for i in range(numTrials):
        ...
        pops[i] = []
        for n in range(300):
            pops[i].append(float(patient.update()))

    pop = []
    for i in range(300):
        pop_tmp = []
        for n in range(numTrials):
            pop_tmp.append(pops[n][i])
        pop.append(sum(pop_tmp) / numTrials)

    pylab.plot(pop, range(300), label='SimpleVirus')
        ...
        if self.getMaxBirthProb() * (1 - popDensity) > random.random():
            for r in self.resistances:
                if self.mutProb > random.random():
                    self.resistances[r] = not self.resistances[r]
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        integer)
        """
        for virus in list(self.getViruses()):
            if virus.doesClear():
        ...
        for virus in list(self.getViruses()):
            try:
                virus_new = virus.reproduce(pop_density, self.prescriptions)
            except NoChildException:
                continue
            self.viruses.append(virus_new)
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    for i in range(numTrials):
        ...
        for n in range(steps):
            ...
            pop_total[n] += patient.update()
            pop_res[n] += patient.getResistPop([drug])

    pop_total = [float(y)/numTrials for y in pop_total]
    pop_res = [float(y)/numTrials for y in pop_res]

    pylab.plot(pop_total, label="Total Virus population")
    pylab.plot(pop_res, label="Resistant Virus population")
